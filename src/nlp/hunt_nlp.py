...
class ThreatHuntingNLP:
    ...
    BEHAVIORAL FOCUS: This NLP module prioritizes behavioral (TTP) hunting over IOC hunting.
    When users provide IOCs, we automatically suggest behavioral pivots.
    """
    ...
    def __init__(self):
        ...
        self.behavioral_focus_mode = True  # Prioritize TTP hunting
    ...
    async def _handle_technique_explanation(self, entities: Dict) -> Dict:
        ...
        if not techniques:
            return {"status": "error", "message": "No MITRE ATT&CK techniques found in query"}

        explanations = {}
        for technique_id in techniques:
            explanations[technique_id] = self._get_technique_explanation(technique_id)

        return {"status": "success", "intent": "explain_technique", "explanations": explanations}

    async def _handle_ioc_analysis(self, entities: Dict) -> Dict:
        """
        Handles IOC analysis requests.

        BEHAVIORAL PIVOT: When analyzing IOCs, we ALWAYS provide behavioral hunting
        alternatives since IOCs are at the bottom of the Pyramid of Pain.
        """
        iocs = entities.get("iocs", [])

        if not iocs:
            return {"status": "error", "message": "No indicators of compromise found in query"}
        ...
        for ioc in iocs:
            ...
            analysis[ioc] = {
                ...
                "behavioral_alternatives": self._suggest_behavioral_hunt(ioc_type),
                "warning": None,
            }

            # Add warning for low-value IOCs
            if pyramid_level in ["hash_values", "ip_addresses", "domain_names"]:
                analysis[ioc]["warning"] = (
                    f"⚠️  {ioc_type.upper()} is at the BOTTOM of the Pyramid of Pain. "
                    f"Adversaries can change this in minutes/hours. "
                    f"Consider the behavioral hunting alternatives provided."
                )
        ...
        return {
            ...
            "recommendation": (
                "💡 IOCs provide context but poor hunting value. "
                "Review the behavioral alternatives for more durable detections."
            ),
        }

    def _suggest_behavioral_hunt(self, ioc_type: str) -> List[Dict]:
        """
        Suggests behavioral hunting alternatives to IOC hunting.

        This is a KEY function that guides users toward better hunting practices.
        """
            ...
            behavioral_alternatives = [
                {
                    ...
                    "hunt_description": "Hunt for C2 beaconing patterns: regular intervals, consistent sizes",
                    "query_type": "Network behavior analysis",
                    "pyramid_level": "TTPs (Top)",
                },
                {
                    "technique": "T1071.004 - DNS Protocol",
                    "hunt_description": "Hunt for DNS tunneling: unusual subdomain lengths, high volumes",
                    "query_type": "Protocol behavior analysis",
                    "pyramid_level": "TTPs (Top)",
                },
            ]
        elif ioc_type in ["hash", "hash_values"]:
            behavioral_alternatives = [
                {
                    "technique": "T1059.* - Command and Scripting Interpreter",
                    "hunt_description": "Hunt for suspicious process chains and parent-child anomalies",
                    "query_type": "Process behavior analysis",
                    "pyramid_level": "TTPs (Top)",
                },
                {
                    "technique": "T1055 - Process Injection",
                    "hunt_description": "Hunt for process injection behaviors regardless of tool",
                    "query_type": "Memory behavior analysis",
                    "pyramid_level": "TTPs (Top)",
                },
            ]
        elif ioc_type in ["url", "uri", "network_artifacts"]:
            behavioral_alternatives = [
                {
                    "technique": "T1505.003 - Web Shell",
                    "hunt_description": "Hunt for web shell behaviors: unusual web server child processes",
                    "query_type": "Web behavior analysis",
                    "pyramid_level": "TTPs (Top)",
                },
                {
                    "technique": "T1041 - Exfiltration Over C2",
                    "hunt_description": "Hunt for data exfiltration patterns in web traffic",
                    "query_type": "Data flow behavior analysis",
                    "pyramid_level": "TTPs (Top)",
                },
            ]

        return behavioral_alternatives
    ...
    def _explain_query(self, spl_query: str, query_type: str) -> str:
        ...
        return explanations.get(
            query_type, "This query searches for suspicious activity based on the provided criteria."
        )
